...
export function FeatureCard({ feature, index }: FeatureCardProps) {
  ...
  return (
    <div className="premium-card h-full" style={{ animationDelay: `${index * 90}ms` }}>
      <Link
        href={targetHref}
        className="group relative block h-full overflow-hidden rounded-3xl border border-white/75 bg-white/85 p-6 shadow-soft transition duration-500 hover:-translate-y-2 hover:border-gold/60 hover:bg-white hover:shadow-2xl focus:outline-none focus:ring-4 focus:ring-campus/15"
